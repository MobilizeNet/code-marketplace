...
func getMatches(extension *noDBExtension, filter Filter) (bool, []int) {
	...
	// If there isn't any filter criteria then it matches
	if filter.Criteria == nil || len(filter.Criteria) <= 0 {
		return true, nil
	}
...
func (db *NoDB) InstallNewExtensions(ctx context.Context) ([]string, string) {
	return db.Storage.InstallNewExtensions(ctx)
}
