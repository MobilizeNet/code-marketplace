...
type InstalledExtensionsResult struct {
...
type Options struct {
	...
	ExternalMarketplace externalmarketplace.ExternalMarketplaceInterface
}
...
func New(options *Options) *API {
	...
	api := &API{
		...
		externalMarketplace: options.ExternalMarketplace,
	}
	...
	r.Get("/installextensions", api.installNewExtensions)
...
func (api *API) extensionQuery(rw http.ResponseWriter, r *http.Request) {
	...
	reqBody, reqError := json.Marshal(query)
	bodyReader := bytes.NewReader(reqBody)
	if reqError == nil {
		status, response := api.externalMarketplace.HttpRequest(http.MethodPost, "vscode/gallery/extensionquery", bodyReader)
		if status == http.StatusOK {
			var extMarketplaceResponse QueryResponse
			err := json.Unmarshal([]byte(response), &extMarketplaceResponse)
			if err == nil {
				results[0].Extensions = append(results[0].Extensions, extMarketplaceResponse.Results[0].Extensions...)
				results[0].Metadata[0].Items[0].Count += extMarketplaceResponse.Results[0].Metadata[0].Items[0].Count
			}
		}
	}
...
func (api *API) installNewExtensions(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	found, message := api.Database.InstallNewExtensions(ctx)
	result := InstalledExtensionsResult{
		Message: message,
		Found:   found,
		Count:   len(found),
	}
	httpapi.Write(rw, http.StatusOK, result)
}
